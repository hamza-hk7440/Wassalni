...
import { getAllStations, createStation, updateStation, deleteStation } from '../../api/admin';
...
function Stations() {
  ...
  return (
    <div className="min-h-screen relative overflow-hidden" style={{ background: `linear-gradient(140deg, ${palette.iceWhite} 0%, ${palette.pureWhite} 55%, ${palette.frostBlue} 100%)` }}>
      ...
      <div className="pointer-events-none absolute top-24 right-0 h-72 w-72 rounded-full blur-3xl opacity-25" style={{ backgroundColor: palette.skyBlue }} />

      <Navbar />
      <div className="relative z-10 pt-24 px-4 md:px-8 pb-8 max-w-7xl mx-auto space-y-6">
        <section className="rounded-3xl border px-6 py-5 md:px-8 md:py-6 shadow-lg" style={{ backgroundColor: 'rgba(255,255,255,0.78)', borderColor: palette.frostBlue }}>
          <div className="flex flex-col gap-2 md:flex-row md:items-center md:justify-between">
            <div>
              <p className="text-xs font-bold uppercase tracking-[0.18em]" style={{ color: palette.classicBlue }}>Transit Admin</p>
              <h1 className="text-3xl md:text-4xl font-black tracking-tight" style={{ color: palette.deepOcean }}>{t('stationsManagement', 'Stations Management')}</h1>
              <p className="text-sm mt-1" style={{ color: palette.textGray }}>{t('stationsSubtitle', 'Add, edit, and organize stations across your network.')}</p>
            </div>
            <div className="rounded-2xl px-4 py-3 min-w-[130px]" style={{ backgroundColor: palette.deepOcean }}>
              ...
              <p className="text-2xl font-extrabold text-white leading-none">{stations.length}</p>
            </div>
          </div>
        </section>

        <form onSubmit={handleSubmit} className="p-6 md:p-7 rounded-3xl border shadow-lg" style={{ backgroundColor: 'rgba(255,255,255,0.9)', borderColor: palette.frostBlue }}>
          <div className="flex items-center justify-between mb-4">
            <h2 className="text-xl font-extrabold" style={{ color: palette.deepOcean }}>{isEditing ? t('editStation', 'Edit a station') : t('addStation', 'Add a station')}</h2>
            {isEditing && <span className="text-xs font-bold px-3 py-1 rounded-full" style={{ backgroundColor: palette.iceWhite, color: palette.classicBlue }}>{t('editMode', 'Edit mode')}</span>}
          </div>
