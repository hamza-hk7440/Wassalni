...
const TicketCard = ({ ticket }) => {
    ...
    return (
        <>
            <div 
                onClick={toggleModal}
                className={`relative mx-auto mb-5 w-full max-w-[750px] overflow-hidden rounded-[15px] border border-[#e2e8f0] bg-white shadow-[0_8px_20px_rgba(0,0,0,0.08)] cursor-pointer transition-transform hover:scale-[1.01] active:scale-100 ${ticket.status === 'expired' || ticket.status === 'used' ? 'grayscale-[0.8] opacity-60' : ''}`}
            >
                ...
                <div className="relative flex items-center justify-around border-b-2 border-dashed border-[#e2e8f0] bg-white px-[30px] py-5">
                    ...
                    <div className="text-[1.3rem] font-bold text-[#6ec1d1]">→</div>
                    <div className="text-center">
                    ...
                    <div className="flex items-center gap-5">
                        <div className="relative flex h-20 w-20 items-center justify-center rounded-lg border-[6px] border-[#f1f5f9] bg-[#f1f5f9] after:font-bold after:text-[#cbd5e1] after:content-['QR']">
                            {ticket.status === 'refunded' && (
                                <div className="absolute inset-0 flex items-center justify-center bg-[#f1f5f9] text-[0.8rem] font-bold text-[#f39c12]">VOID</div>
                            )}
                        </div>
                        <div>
                            ...
                            <p className="text-[0.65rem] text-[#6ec1d1] font-bold uppercase mt-1">Click to enlarge</p>
                        </div>
                    </div>
                </div>
                <div className="flex justify-between border-t border-[#e2e8f0] bg-[#f8fafc] px-[30px] py-[15px]">
                    ...
                    <div className="flex flex-col text-right">
                        ...
                        <span className={`text-[0.85rem] font-semibold ${ticket.status === 'expired' ? 'font-extrabold text-[#e53e3e]' : 'text-[#2c3e50]'}`}>
            ...
            {isModalOpen && (
                ...
                >
                    ...
                    >
                        ...
                        <h3 className="text-[#1e5470] font-bold text-lg mb-2">Scan Ticket</h3>
                        <p className="text-gray-400 text-sm mb-6">Present this to the driver or scanner</p>
                        <div className="relative flex h-56 w-56 items-center justify-center rounded-2xl border-[12px] border-[#f1f5f9] bg-[#f1f5f9] after:text-4xl after:font-bold after:text-[#cbd5e1] after:content-['QR'] mb-6">
                            {ticket.status === 'refunded' && (
                                <div className="absolute inset-0 flex items-center justify-center bg-[#f1f5f9]/80 text-xl font-bold text-[#f39c12]">VOID</div>
                            )}
                        </div>
                        ...
                        <button 
                            onClick={toggleModal}
                            className="mt-8 w-full py-3 bg-[#1e5470] text-white rounded-xl font-bold hover:bg-[#1a4a63] transition-colors"
                        >
                            Done
                        </button>
                    </div>
