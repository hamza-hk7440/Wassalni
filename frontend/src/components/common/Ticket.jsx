...
const TicketCard = ({ ticket }) => {
  ...
  const isCancelled = Boolean(ticket?.is_cancelled);
  const delayMinutes = Number(ticket?.delay_minutes ?? 0);
  const disruptionMessage = String(ticket?.disruption_message || "").trim();
  ...
  return (
    <>
      ...
      >
        <div
          className={`flex items-center justify-between px-[25px] py-3 text-[0.8rem] font-bold uppercase tracking-[1.2px] text-white ${statusStyles[ticket.status] || "bg-gray-500"}`}
        >
        ...
        {(isCancelled || delayMinutes > 0 || disruptionMessage) && (
          <div className="px-[25px] py-2 border-b border-[#e2e8f0] bg-[#fff7ed]">
            <div className="flex flex-wrap items-center gap-2">
              {isCancelled && (
                <span className="rounded-full bg-[#fee2e2] px-2 py-1 text-[0.65rem] font-bold uppercase tracking-wide text-[#b91c1c]">
                  Cancelled Trip
                </span>
              )}
              {delayMinutes > 0 && (
                <span className="rounded-full bg-[#ffedd5] px-2 py-1 text-[0.65rem] font-bold uppercase tracking-wide text-[#c2410c]">
                  Delayed +{delayMinutes}m
                </span>
              )}
            </div>
            {disruptionMessage && (
              <p className="mt-1 text-[0.75rem] font-medium text-[#7c2d12]">
                {disruptionMessage}
              </p>
            )}
          </div>
        ...
        <div className="relative flex items-center justify-around border-b-2 border-dashed border-[#e2e8f0] bg-white px-[30px] py-5">
          ...
          <div className="text-center">
            <label className="block text-[0.65rem] font-bold uppercase text-[#94a3b8]">
              From
            </label>
            <span className="text-[1.2rem] font-extrabold text-[#1e5470]">
              {ticket.from_station || "Departure"}
            </span>
          </div>
          <div
            className={`text-[1.3rem] font-bold ${ticket.status === "used" ? "text-[#e53e3e]" : "text-[#6ec1d1]"}`}
          >
            →
          </div>
          <div className="text-center">
        ...
        <div className="flex flex-col items-center bg-white p-5">
          <div className="flex items-center gap-5">
            ...
            <div>
              ...
              <p
                className={`text-[0.65rem] font-bold uppercase mt-1 ${ticket.status === "used" ? "text-[#e53e3e]" : "text-[#6ec1d1]"}`}
              >
                {ticket.status === "used"
                  ? "Ticket Processed"
                  : "Click to enlarge"}
        ...
        <div className="flex justify-between border-t border-[#e2e8f0] bg-[#f8fafc] px-[30px] py-[15px]">
          <div className="flex flex-col">
            <label className="text-[0.6rem] font-bold uppercase text-[#94a3b8]">
              Valid From
            </label>
            <span className="text-[0.85rem] font-semibold text-[#475569]">
          ...
          <div className="flex flex-col text-right">
            ...
            <span
              className={`text-[0.85rem] font-semibold ${ticket.status === "expired" || ticket.status === "used" ? "font-extrabold text-[#e53e3e]" : "text-[#2c3e50]"}`}
            >
      ...
      {isModalOpen && (
        ...
        >
          ...
          >
            ...
            <h3
              className={`font-bold text-lg mb-2 ${ticket.status === "used" ? "text-[#e53e3e]" : "text-[#1e5470]"}`}
            >
              {ticket.status === "used" ? "Ticket Already Used" : "Scan Ticket"}
            </h3>
            {(isCancelled || delayMinutes > 0 || disruptionMessage) && (
              <div className="mb-3 w-full rounded-xl border border-[#fed7aa] bg-[#fff7ed] px-3 py-2 text-left">
                {isCancelled && (
                  <p className="text-[0.72rem] font-bold uppercase text-[#b91c1c]">
                    Cancelled Trip
                  </p>
                )}
                {delayMinutes > 0 && (
                  <p className="text-[0.72rem] font-bold uppercase text-[#c2410c]">
                    Delayed +{delayMinutes}m
                  </p>
                )}
                {disruptionMessage && (
            ...
            <p className="text-gray-400 text-sm mb-6 text-center">
              {ticket.status === "used"
                ? "This ticket has been scanned and is no longer valid for travel."
                : "Present this to the driver or scanner"}
            </p>
            ...
            <div className="text-center">
              <span className="block font-mono text-xl tracking-[4px] font-bold text-[#1e5470]">
                {ticket.qr_code}
              </span>
              <span className="text-xs text-gray-400 mt-1 block uppercase font-bold tracking-widest">
                {ticket.type} Ticket
              </span>
            </div>
            <button
              onClick={toggleModal}
              className={`mt-8 w-full py-3 text-white rounded-xl font-bold transition-colors ${ticket.status === "used" ? "bg-[#e53e3e] hover:bg-red-700" : "bg-[#1e5470] hover:bg-[#1a4a63]"}`}
            >
              Done
            </button>
          </div>
